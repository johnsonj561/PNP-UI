...
public class GCodeCommand {
	...
	public GCodeCommand(String instruction, double width, double depth, double height) {
		...
		fCommand = false;
		mCommand = false;
		//initialize validCommand and picReturn to false
		isPicReturn = false;
		...
		//also have to check if it is a PIC return value
		String picReturnRegex = "^X\\d+\\.\\d+\\s*Y\\d+\\.\\d+\\s*Z\\d+\\.\\d+\\s*$";
		
		//if it matches expected G Code Command Format
		if(gCodeString.matches(gCodeRegex)){
			isValidCommand = true;
		}
		//else if it matches expected Pic Return Value
		else if(gCodeString.matches(picReturnRegex)){
			isPicReturn = true;
			System.out.println("Valid Pic Return Found");
		}
		//else command must be invalid
		else{
			isValidCommand = false;
			System.out.println("Fundamental Formatting Failed: " + gCodeString);
		}
		
		//if we passed fundamental testing - then dig deeper for more info
		if(isValidCommand || isPicReturn){
	...
	/**
	 * Returns true if command is consistent with PIC return values
	 * @return true if this command is consistent with PIC return values
	 */
	public boolean isPicReturnValue(){
		return isPicReturn;
	}
	...
	//boolean - true if matches PIC return value
	private boolean isPicReturn;
	//store values
	private int gValue;
	private int mValue;
